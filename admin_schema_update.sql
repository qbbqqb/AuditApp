-- Update the user_role enum to include 'admin'
-- Note: In PostgreSQL, you can't directly modify an enum, so we need to add the new value
ALTER TYPE user_role ADD VALUE 'admin' BEFORE 'client_safety_manager';

-- Update the user_has_finding_access function to handle admin role
...
BEGIN
  ...
  -- Admin has access to all findings
...
-- Update the get_dashboard_metrics function to handle admin role
...
BEGIN
  ...
  -- Admin can see all findings, others see filtered findings
  IF user_role = 'admin' THEN
    RETURN QUERY
    SELECT 
